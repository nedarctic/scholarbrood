...
export default function OrderForm() {
  ...
  const [selectedServiceType, setSelectedServiceType] = useState<string>(
    isPreSelected ? selectedService : "General Consultation"
  );
  const [paypalAmount, setPaypalAmount] = useState<number>(parseFloat(amount) || 0);

  // Update PayPal amount when service changes
  useEffect(() => {
    const pricing = SERVICE_PRICING[selectedServiceType as keyof typeof SERVICE_PRICING];
    if (pricing) {
      setPaypalAmount(pricing.amount);
    }
  }, [selectedServiceType]);
  ...
  const paypalUrl = `https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=scholarbrood1@gmail.com&item_name=${encodeURIComponent(
    selectedServiceType
  )}&amount=${paypalAmount}&currency_code=USD&no_shipping=1&no_note=1&lc=US&bn=PP-BuyNowBF`;

  const pricing = SERVICE_PRICING[selectedServiceType as keyof typeof SERVICE_PRICING] || { amount: 0, unit: "service" };
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full">
      ...
      >
        ...
        {/* Dynamic PayPal Payment Option */}
        {paypalAmount > 0 && (
          <div className="mt-10 pt-6 border-t border-white/20 text-center space-y-3">
            <p className={`${oswald.className} text-sm opacity-80`}>
              Suggested starting payment for <strong>{selectedServiceType}</strong>:{" "}
              <strong>${paypalAmount}</strong> {pricing.unit === "page" ? "/ page" : ""}
            </p>
        ...
        {/* Alternative PayPal Link (for fallback) */}
        {/* {paypalAmount > 0 && (
          <div className="mt-4 text-center">
            <a
              ...
              className={`${oswald.className} inline-flex items-center gap-3 px-6 py-3 rounded-full bg-[#0070BA] text-white text-sm font-bold hover:bg-[#005ea6] transition`}
            >
